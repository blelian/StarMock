...
async function testEndpoint(name, url, options = {}) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        `   ❌ ${response.status} - ${data.error?.message || 'Failed'}`
      )
    }
...
async function testCompleteUserJourney() {
  ...
  // Test 4: Logout
  await testEndpoint('User Logout', `${BASE_URL}/auth/logout`, {
    method: 'POST',
  })
  ...
  assert(statusAfterLogout.ok, 'Post-logout auth status check successful')
  assert(
    statusAfterLogout.data?.isAuthenticated === false,
    'User is logged out'
  )
  ...
  if (questionsResult.data?.questions?.length > 0) {
    ...
    assert(
      singleQuestionResult.data?.question?.id === questionId,
      'Question ID matches'
    )
  } else {
  ...
  const questionIds =
    questionsResult.data?.questions?.slice(0, 3).map((q) => q.id) || []
  ...
  assert(
    sessionResult.data?.session?.status === 'in_progress',
    'Session status is in_progress'
  )
  assert(
    sessionResult.data?.session?.questionCount === 3,
  ...
  assert(
    sessionDetailsResult.data?.session?.id === sessionId,
    'Session ID matches'
  )
...
Result: Response times improved by 60%, customer satisfaction increased, and I was promoted to mid-level developer.`,
  ...
  assert(
    allResponsesResult.data?.responses?.length === 3,
    'All 3 responses returned'
  )
  ...
  assert(
    completeResult.data?.session?.status === 'completed',
    'Session status is completed'
  )
  assert(
    completeResult.data?.session?.duration !== undefined,
    'Duration calculated'
  )

  // Test 8: Feedback Generation (async — poll until ready)
  console.log('\n📋 Test Suite 4: Feedback Generation & Analysis')
  console.log('-'.repeat(60))

  // The feedback worker processes jobs asynchronously. Poll until
  // feedback reports are available or a generous timeout elapses.
  const FEEDBACK_POLL_INTERVAL = 2000 // ms
  const FEEDBACK_POLL_TIMEOUT = 60000 // ms
  let feedbackResult
  const feedbackStart = Date.now()

  while (Date.now() - feedbackStart < FEEDBACK_POLL_TIMEOUT) {
    feedbackResult = await testEndpoint(
      'Poll Feedback',
      `${BASE_URL}/sessions/${sessionId}/feedback`
    )
    if (feedbackResult.ok && feedbackResult.data?.feedback?.length > 0) {
      break
    }
    console.log('   ⏳ Feedback not ready yet, retrying...')
    await new Promise((r) => setTimeout(r, FEEDBACK_POLL_INTERVAL))
  }

  assert(feedbackResult.ok, 'Feedback generated successfully')
  assert(
    feedbackResult.data?.feedback?.length === 3,
    'Feedback for all 3 responses'
  )
  assert(
    feedbackResult.data?.feedback[0]?.scores?.overall !== undefined,
    'Overall score present'
  )
  assert(
    feedbackResult.data?.feedback[0]?.rating !== undefined,
    'Rating present'
  )
  assert(
    feedbackResult.data?.feedback[0]?.strengths?.length > 0,
    'Strengths provided'
  )
  assert(
    feedbackResult.data?.feedback[0]?.suggestions?.length > 0,
  ...
  assert(
    cachedFeedbackResult.data?.feedback?.length === 3,
    'Same feedback returned'
  )
  ...
  assert(
    historyResult.data?.sessions?.length > 0,
    'At least one session in history'
  )
  assert(
    historyResult.data?.pagination !== undefined,
  ...
  const historyHasSession = historyResult.data?.sessions?.some(
    (s) => s.id === sessionId
  )
  ...
  assert(
    !notFoundResult.ok && notFoundResult.status === 404,
  ...
  assert(
    !invalidResponseResult.ok && invalidResponseResult.status === 400,
    'Validates required fields'
  )

  // Try to complete already completed session (idempotent — returns 200)
  ...
  assert(doubleCompleteResult.ok, 'Idempotent re-completion succeeds')
  assert(
    doubleCompleteResult.data?.message?.includes('already completed'),
    'Server confirms session was already completed'
  )
  ...
  assert(
    !unauthResult.ok && unauthResult.status === 401,
