...
function calculateDetailScore(structure) {
  ...
  if (
    structure.avgWordsPerSentence >= 15 &&
    structure.avgWordsPerSentence <= 25
  ) {
    score += 10
  } else {
    score += 5
...
function generateSuggestions(scores, structure) {
  ...
  if (scores.situation < 50) {
    suggestions.push(
      'Add more context about the situation. Describe when and where this occurred, and what the circumstances were.'
    )
  }

  if (scores.task < 50) {
    suggestions.push(
      'Clarify your specific role and responsibilities. What were you expected to accomplish?'
    )
  }

  if (scores.action < 50) {
    suggestions.push(
      'Provide more detail about the actions you took. Use action verbs to describe what you specifically did.'
    )
  }

  if (scores.result < 50) {
    suggestions.push(
      'Emphasize the outcomes and impact of your actions. Include metrics or specific achievements when possible.'
    )
  }
  ...
  if (structure.wordCount < 50) {
    suggestions.push(
      'Your response is quite brief. Aim for 100-200 words to provide sufficient detail.'
    )
  } else if (structure.wordCount > 300) {
    suggestions.push(
      'Consider being more concise. Focus on the most impactful details of your STAR response.'
    )
  }

  if (!structure.hasMultipleParagraphs && structure.wordCount > 100) {
    suggestions.push(
      'Break your response into paragraphs to improve readability (e.g., one for each STAR component).'
    )
  }

  if (structure.sentenceCount < 4) {
    suggestions.push(
      'Add more sentences to elaborate on each part of your story.'
    )
  }
  ...
  if (maxScore - minScore > 40) {
    suggestions.push(
      'Try to balance all four STAR components equally in your response.'
    )
  }
...
export function evaluateResponse(responseText) {
  ...
  const situationAnalysis = calculateComponentPresence(
    responseText,
    'situation'
  )
