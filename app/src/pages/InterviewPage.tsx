...
const InterviewPage = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: 'ai', content: "Hello! I'm your AI interviewer today. Are you ready to start our session? Tell me a bit about the role you're applying for." }
    ]);
    ...
    const handleSendMessage = async (text: string = inputText) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'ai', content: "I'm sorry, I'm having trouble connecting to my brain right now. Please try again." }]);
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <header className="glass-card m-4 p-4 flex items-center justify-between sticky top-0 z-50">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-text-muted">Interview Session • {user?.name}</p>
                    </div>
                </div>
                ...
                <div className="flex-1 glass-card flex flex-col overflow-hidden">
                            ...
                            {messages.map((msg, i) => (
                                ...
                                >
                                    <div className={`flex gap-3 max-w-[80%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                        ...
                                        <div className={`p-4 rounded-2xl ${msg.role === 'user' ? 'bg-primary text-white rounded-tr-none' : 'bg-bg-card border border-glass-border rounded-tl-none'
                                            }`}>
                    ...
                    <div className="p-4 bg-bg-dark/50 border-t border-glass-border">
                        <div className="relative flex items-center gap-2">
                            ...
                            <button
                                onClick={toggleMic}
                                className={`p-3 rounded-xl transition-all ${isListening ? 'bg-error text-white scale-110' : 'bg-bg-card hover:bg-primary/20 text-text-muted hover:text-primary'
                                    }`}
                            >
                    ...
                    <div className="glass-card p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <div className="w-full bg-bg-dark h-1.5 rounded-full overflow-hidden">
                                    <motion.div initial={{ width: 0 }} animate={{ width: '72%' }} className="bg-accent h-full" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="glass-card p-6 flex-1">
                        ...
                        <div className="space-y-3">
                            {[
                                ...
                                <div key={i} className="text-sm p-3 bg-bg-dark/50 rounded-lg border border-glass-border">
                                    {s}
                                </div>
                            ))}
                        </div>
